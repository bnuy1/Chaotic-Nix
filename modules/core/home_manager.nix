          ...
          if user.minimal or false then
            {
              ...
              home.homeDirectory =
                if user ? homeDirectory && user.homeDirectory != null && user.homeDirectory != "" then
                  user.homeDirectory
                else
                  "/home/${user.name}";
            ...
            let
              userAliases =
                if !(builtins.elem "wheel" (user.extraGroups or [ ])) then
                  builtins.removeAttrs systemAliases [ "nrs" ]
                else
                  systemAliases;
            in
            {
              ...
              imports = lib.optional (!(user.minimal or false)) ../home ++ [
                (
                  let
                    nixvimFile =
                      if user ? nixvimConfig && user.nixvimConfig != null then
                        user.nixvimConfig
                      else if builtins.pathExists ../home/nixvim/${user.name}.nix then
                        user.name
                      else
                        "base";
                    nixvimPath = ../home/nixvim/${nixvimFile}.nix;
                  in
                  if builtins.pathExists nixvimPath then
                    nixvimPath
                  else
                    throw ''
              ...
              home.packages = map (name: lib.getAttrFromPath (lib.splitString "." name) pkgs) (
                user.extraPkgs or [ ]
              );
              ...
              programs.git = {
                ...
                // lib.optionalAttrs (user ? githubSSHKey) {
                  # github ssh instead of https credential auth
              ...
              programs.fish.shellAliases = userAliases // { ls = "ls --color"; } // (user.shellAliases or { });
            };
      }) vars.users
    );
  };
