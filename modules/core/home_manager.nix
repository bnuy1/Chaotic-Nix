  ...
  home-manager = {
    ...
    users = builtins.listToAttrs (map (user: {
      ...
      value = { pkgs, ... }: {
        ...
        home.homeDirectory = if user ? homeDirectory && user.homeDirectory != null then user.homeDirectory else "/home/${user.name}";
        ...
        imports =
          lib.optional (!(user.minimal or false)) ../home
          ++ [(
            let
              nixvimFile = if user ? nixvimConfig && user.nixvimConfig != null then
                user.nixvimConfig
        ...
        home.packages = map (name: lib.getAttrFromPath (lib.splitString "." name) pkgs) (user.extraPkgs or []);
