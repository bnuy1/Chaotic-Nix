...
if args.termscheme is not None:
    ...
    # Rotate the six vivid ANSI hues together with the wallpaper accent.
    # The anchor is the scheme's primary palette hue, so the terminal also
    # follows the palette mode (content/expressive/fidelity/...). Fixed
    # 40deg offsets keep the six hues distinct and vivid instead of washing
    # everything toward one accent hue (what a high harmony value does).
    hue_offsets = {
        'term1': 0,    'term2': 40,  'term3': 80,
        'term4': 120,  'term5': 160, 'term6': 200,
        'term9': 0,    'term10': 40, 'term11': 80,
        'term12': 120, 'term13': 160, 'term14': 200,
    }
    for color, val in term_source_colors.items():
        ...
        elif color in hue_offsets:
            source_hct = Hct.from_int(hex_to_argb(val))
            hue = sanitize_degrees_double(primary_hct.hue + hue_offsets[color])
            chroma = max(source_hct.chroma, 90)
            harmonized = Hct.from_hct(hue, chroma, source_hct.tone).to_int()
        else:
